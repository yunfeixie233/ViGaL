    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, labels in self.prompts_dataloader:
                ...
                ## log acc change
                ## log acc change
                if experiences and all("accuracy_rewards" in exp.info for exp in experiences):
                    ...
                    hard_counts = accuracy_counts[0][-1] if accuracy_counts else 0
                    easy_counts = accuracy_counts[-1][-1] if accuracy_counts else 0
                    mid_counts = sum(acc[-1] for acc in accuracy_counts[1:-1]) if len(accuracy_counts) > 2 else 0
                    ...
                    print("Warning: No experiences or missing accuracy_rewards")
                    status["hard_counts"] = 0
                    status["easy_counts"] = 0
                    status["mid_counts"] = 0

                ## log the entropy for a group of responses
                if experiences and all(hasattr(experience, "action_log_probs") and hasattr(experience, "action_mask") for experience in experiences):
                    ...
                    print("Warning: Missing action_log_probs or action_mask for entropy calculation")
                    status["entropy_per_prompt"] = 0.0

                # Handle accuracy_rewards_original
                if 'accuracy_rewards_original' in locals() and accuracy_rewards_original is not None:
                    status["accuracy_rewards_original"] = accuracy_rewards_original
                else:
                    status["accuracy_rewards_original"] = 0.0
                if "kl" in status:
