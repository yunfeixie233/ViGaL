...
(function () {
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_min-content_1fr] gap-4 mb-6">
                ...
                <div class="thoughts-panel player1-border flex flex-col">
                    ...
                    <div class="flex-1 p-4 overflow-auto">
                        <div id="player1Thoughts" class="font-mono text-xs space-y-1 whitespace-pre-wrap"></div>
                    </div>
                ...
                <div class="thoughts-panel player2-border flex flex-col">
                    ...
                    <div class="flex-1 p-4 overflow-auto">
                        <div id="player2Thoughts" class="font-mono text-xs space-y-1 whitespace-pre-wrap"></div>
                    </div>
                    <div class="p-3 border-t border-gray-100 flex justify-between">
    ...
    function boot(jsonPath) {
        ...
        /* ---------------- rationale helpers ---------------- */
        const formatMultiline = (txt) => txt.trim().replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
        ...
        const thought = (rd,pid)=>{
            const arr=[];
            if(!rd.alive?.[pid]){
                arr.push(`Round ${rd.round_number}: <strong class="text-red-600">ELIMINATED</strong>`);
                arr.push(`Score: ${rd.scores[pid]}`);
                return arr;
            }
            const mv=(rd.move_history||[]).at(-1)?.[pid];
            if(mv){
                arr.push(`<strong>Round ${rd.round_number}</strong>`);
                const txt = mv.rationale || '';
                const think = extractTag(txt,'think');
                const best  = extractTag(txt,'best_answer');
                const worst = extractTag(txt,'worst_answer');
                if(think)  arr.push(`💭 <strong>Thought</strong><br>${formatMultiline(think)}`);
                if(best)   arr.push(`✅ <strong>Best</strong> ${best}`);
                if(worst)  arr.push(`⚠️ <strong>Worst</strong> ${worst}`);
            }
            arr.push(`Score: ${rd.scores[pid]}`);
        ...
        function render(){
            ...
            ['1','2'].forEach(pid=>{
                ...
                document.getElementById(`player${pid}Thoughts`).innerHTML=thought(rd,pid).map(t=>`<p class="mb-1">${t}</p>`).join('');
            });
