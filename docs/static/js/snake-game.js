...
(function () {
    ...
    function boot(jsonPath) {
        ...
        const thought = (rd,pid)=>{
            const arr=[];
            if(!rd.alive?.[pid]){arr.push(`Round ${rd.round_number}: ELIMINATED`);arr.push(`Score: ${rd.scores[pid]}`);return arr;}
            const mv=(rd.move_history||[]).at(-1)?.[pid];
            if(mv){arr.push(`Round ${rd.round_number}: Move ${mv.move}`);
                const txt=mv.rationale||'';
                const m=(tag)=>txt.match(new RegExp(`<${tag}>([\s\S]*?)<\/${tag}>`));
                m('think')&&arr.push(`Thought: ${m('think')[1].trim()}`);
                m('best_answer')&&arr.push(`Best move: ${m('best_answer')[1].trim()}`);
                m('worst_answer')&&arr.push(`Worst move: ${m('worst_answer')[1].trim()}`);
            }
