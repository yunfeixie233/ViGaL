...
if [ "$PET_NODE_RANK" -eq 0 ]; then
  RAY_ADDRESS="http://127.0.0.1:$RAY_DASHBOARD_PORT" ray job submit \
    ...
    --colocate_all_models \
    --pretrain ${MODEL_PATH}  \
    --remote_rm_url /lustre/fsw/portfolios/av/users/shiyil/yunfei/new/MM-EUREKA/examples/scripts/reward_func_qwen_rotation.py \
    ...
    --init_kl_coef 0.0 \
    --prompt_data ${BASE_DIR}/${EXP}/metadata_train.jsonl \
    --input_key message \
    --normalize_reward \
    --flash_attn \
